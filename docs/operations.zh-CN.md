...
控制台独立的“遥控”标签页在操作员显式启用后才会发布受限的 `/cmd_vel` 命令。它要求
基础栈或本机建图正在运行，且 `/scan`、`/odom`、`/imu`、`/tf` 均活跃、导航已停止。
可按住方向按钮，或按住 `W/↑`、`S/↓`、`A/←`、`D/→` 移动；Space 或 `0` 会立即停止。
遥控页可为各方向和停止操作在本机保存一个额外的自定义按键。可切换到“地图视图”并继续使用
映射按键遥控；窗口失焦、禁用遥控、前提条件失效或退出控制台时都会发送零速度。它没有避障能力，必须保留实体急停
和现场监护，且不能与其他 `/cmd_vel` 控制源同时使用。巡检会为路线中的每个点发送 Nav2
目标，最后返回配置的起始点。
